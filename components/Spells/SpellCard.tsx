...
import { useUpdateSpellMutation } from 'services/spells/spell';
...
export const SpellCard = ({ spell }: SpellCardProps) => {
  ...
  const { mutate: updateSpell } = useUpdateSpellMutation(spell.level);
  ...
  return (
    <View className="bg-white rounded-lg p-3 mb-3 border border-gray-200">
      <View className="flex flex-row justify-between items-start mb-2">
        <View className="flex-1">
          <View className="flex flex-row items-center gap-1">
            ...
            {spell.concentration && (
              <AlertCircle size={16} color="orange" title="Concentration" />
            )}
          </View>
        ...
        {spell.level > 0 && (
          <View className="flex flex-col items-center ml-2">
            <Text className="text-xs text-gray-500 mb-1">{i18n.t('spells.prepared')}</Text>
            <Switch
      ...
      <View className="flex flex-row flex-wrap gap-y-1 gap-x-3 mb-2">
        <View>
          <Text className="text-xs font-bold">{i18n.t('spells.castingTime')}:</Text>
        ...
        <View>
          <Text className="text-xs font-bold">{i18n.t('spells.duration')}:</Text>
          <Text className="text-sm">{spell.duration}</Text>
        </View>
        <View>
          <Text className="text-xs font-bold">{i18n.t('spells.components')}:</Text>
          <Text className="text-sm">{getComponentsString()}</Text>
        </View>
        {spell.innateAmount > 0 && (
          <View>
            <Text className="text-xs font-bold">{i18n.t('spells.innateAmount')}:</Text>
            <Text className="text-sm">{spell.innateAmount}</Text>
          </View>
        )}
      </View>
          ...
          {expanded ? (
            <View>
              ...
              {spell.higherLevelDescription && (
                <View className="mt-2">
                  <Text className="text-xs font-bold">{i18n.t('spells.higherLevelDescription')}:</Text>
                  <Text className="text-sm text-gray-800">{spell.higherLevelDescription}</Text>
                </View>
              )}
              <TouchableOpacity onPress={() => setExpanded(false)} className="mt-2 flex-row justify-center">
            ...
            <View>
              ...
              <TouchableOpacity onPress={() => setExpanded(true)} className="mt-1 flex-row justify-center">
